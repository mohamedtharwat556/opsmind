...
const Reports = () => {
  ...
  useEffect(() => {
    Promise.all([
      fetch('http://localhost:5000/api/cases').then(r => r.json()).then(setCases).catch(() => setCases([])),
      fetch('http://localhost:5000/api/users').then(r => r.json()).then(setUsers).catch(() => setUsers([])),
      fetch('http://localhost:5000/api/sops').then(r => r.json()).then(setSops).catch(() => setSops([]))
