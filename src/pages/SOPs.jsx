...
const SOPs = () => {
  ...
  const handleAiGenerate = (e) => {
    ...
    fetch('http://localhost:5000/api/ai/generate-sop', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  const handleSuggestEdit = (e) => {
    ...
    fetch(`http://localhost:5000/api/sops/${currentSOP.id}/suggest-edit`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
  ...
  const nextPlaybookStep = () => {
    if (currentStepIndex < currentSOP.steps.length - 1) {
      ...
      // Post log to backend
      fetch('http://localhost:5000/api/playbooks/execute', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          sopId: currentSOP.id,
