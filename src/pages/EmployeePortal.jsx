...
const EmployeePortal = () => {
  ...
  useEffect(() => {
    fetch(`${API_URL}/api/sops`).then(r => r.json()).then(setSops).catch(() => {});
    fetch(`${API_URL}/api/articles`).then(r => r.json()).then(setArticles).catch(() => {});
    fetch(`${API_URL}/api/announcements`).then(r => r.json()).then(setAnnouncements).catch(() => {});
  }, []);

  useEffect(() => {
    if (!searchQuery.trim()) { setSearchResults(null); return; }
    const t = setTimeout(() => {
      fetch(`${API_URL}/api/search?q=${searchQuery}`)
